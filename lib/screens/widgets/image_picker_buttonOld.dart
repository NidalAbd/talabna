...
class ImagePickerButtonState extends State<ImagePickerButton> {
  ...
  Future<void> _pickVideo() async {
    ...
    if (pickedFile != null) {
      ...
      if (_pickedImages.length + 1 > maxImages) {
        ...
        if (videoFile.lengthSync() <= maxSizeBytes && videoFile.path.toLowerCase().endsWith('.mp4')) {
  ...
  Future<void> _submitLocalImages() async {
    final List<Photo> localImages = _pickedImages.where((photo) => _localImages.contains(photo.src)).toList();
                  ...
                  children: [
                    ...
                    Image(
                      image: (_localImages.isNotEmpty && _localImages[index] != null)
                          ...
                          ? ThumbnailImage(videoPath: _localImages[index]!) as ImageProvider
